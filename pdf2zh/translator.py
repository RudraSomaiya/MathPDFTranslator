...
def translate_with_nllb(text, source_lang='en', target_lang='te'):
    src = LANG_CODE_MAP.get(source_lang, 'eng_Latn')
    tgt = LANG_CODE_MAP.get(target_lang, 'tel_Telu')

    tokenizer.src_lang = src
    inputs = tokenizer(text, return_tensors="pt", padding=True, truncation=True)
    translated_tokens = model.generate(**inputs, forced_bos_token_id=tokenizer.lang_code_to_id[tgt])
...
class NLLBTranslator(BaseTranslator):
    ...
    def do_translate(self, text):
        src_lang = self.lang_map.get(self.lang_in, "eng_Latn")
        tgt_lang = self.lang_map.get(self.lang_out, "tel_Telu")
